...
def borrowbook(request):
    if request.method == 'POST':
        add_book = BorrowForm(request.POST)
        if add_book.is_valid():
            add_book.save()
            return redirect('borrowedbook')
    context = {'form' : BorrowForm()}
    return render(request, 'pages/borrow.html', context)

def borrowedbook(request):
    books = BorrowedBook.objects.all()
    context={'books':books}
    return render(request, 'pages/borrowedbook.html',context)

def extendborrow(request,id):
    book_id = BorrowedBook.objects.get(id=id)
    if request.method == 'POST':
        book_save = BorrowForm(request.POST, instance=book_id)
        if book_save.is_valid():
            book_save.save()
            return redirect('borrowedbook')
    else:
        book_save = BorrowForm(instance=book_id)
    context = {'form4' : book_save}
    return render(request, 'pages/extendborrow.html', context)

def returnborrow(request,id):
    book_id = get_object_or_404(BorrowedBook, id=id)
    if request.method == 'POST':
        book_id.delete()
        return redirect('borrowedbook')
    return render(request, 'pages/borrowedbook.html')